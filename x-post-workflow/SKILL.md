...
| bird CLI | `/usr/local/bin/bird` |
| humanizer | `humanizer` skill (需另行安装) |
...
### 2026-06-15
- Step 4 整合 humanizer-skill（已安装），替代手动检查清单
- Step 7 改用脚本发布（Linux: Playwright版, macOS: AppleScript版）
- 新增 `scripts/post-with-schedule.py`（Linux/跨平台Playwright版）
- 修复日期时间前导零Bug（X的select value需匹配 "05" 而非 "5"）
